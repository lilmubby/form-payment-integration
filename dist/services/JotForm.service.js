...
class JotformService {
    async submitForm(paymentData) {
        logger.info("Submitting form to Jotform", { reference: paymentData.paymentReference });
        try {
            const response = await axios.post(`https://api.jotform.com/form/${config.JOTFORM_FORM_ID}/submissions?apiKey=${config.JOTFORM_API_KEY}`, {
                submission: {
                    ...paymentData,
                },
            });
    ...
    async deleteSubmission(submissionId) {
        try {
            const response = await axios.delete(`https://api.jotform.com/submission/${submissionId}?apiKey=${config.JOTFORM_API_KEY}`);
